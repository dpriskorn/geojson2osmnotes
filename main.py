...
"Här ska finnas en badplats enligt Topo50 från Lantmäteriet. Tyvärr finns varken namn eller beständigt id från Lantmäteriet.
Kolla ev med kommunen om vad denna badplats heter och om de är beredda att släppa öppna data om sina badplatser. Kolla även gärna upp på plats om skyltning finns.
Om badplats med skyltning upphittas så får ni gärna skapa en issue så vi kan lägga till den i Wikidata också här: länk till svenska badvatten repo"

Fråga chatgpt hur jag bäst beräknar närheten av badplatser i osm för varje punkt i topo50 dataframen.
Min gissning är att göra en ny dataframe med distanskolumn för varje topo50 punkt och beräkna distansen till alla badplatser i osm och filtrera bort allt över 100 m.
Är det feasible?
"""
import logging
...
class GeojsonHandler(BaseModel):
    ...
    def iterate_source_features(self):
        ...
        for index, row in self.source_df.iterrows():
            ...
            osm_distance_df['distance_to_point'] = self.osm_df.apply(self.calculate_distance, axis=1, args=(geometry,))
            # Remove rows with distance over 100 meters
            osm_distance_df = osm_distance_df.loc[osm_distance_df['distance_to_point'] <= 100]

            # Sort by ascending distance
            osm_distance_df = osm_distance_df.sort_values(by='distance_to_point', ascending=True)
    ...
    def setup_argparse_and_get_filename(self):
        ...
        parser.add_argument("--source-geojson", required=True, help="Source geojson file")
